...
@gui.composable
def button(text, font_size=20):
    pressed = gui.state(False)
    print('P', pressed.value, pressed)
    @gui.drawing
    def _draw_(ui, comp):
        print(pressed.value)
        bb = comp.shape
    ...
    def _down_(x, y, button):
        pressed.value = True
        print('R', pressed.value, pressed)
    @gui.listen(gui.e_button_up)
    def _up_(x, y, button):
        pressed.value = False
        print('D', pressed.value, pressed)
...
def app(): 
    ...
    hmm = button(f"hello {counter.value}")
    hmm.reshape(gui.Box(10, 10, 100, 20))
    @hmm.listen(gui.e_button_down)
    def _click_(x, y, button):
        counter.value += 1
    src = gui.state('foobar')
    haa = textbox(src.value, src.set_state)
    haa.reshape(gui.Box(10+110, 10, 100, 20))
...
class Editor:
    ...
    def ui(self):
        ...
        other = self.widget("rollernote", 1200, 700, gui.GUI, app)
