...
from ctypes import c_bool, c_double, c_float, c_int, c_size_t, c_uint, c_uint32, c_uint64, c_ulong
...
class LV2_Log_Handle(Structure):
    __slots__ = ['handle', 'printf', 'vprintf']
    _fields_ = [
        ('handle', c_void_p),
        ('printf', CFUNCTYPE(None, c_void_p, LV2_URID, c_char_p)),
        ('vprintf', CFUNCTYPE(None, c_void_p, LV2_URID, c_char_p)) ]

LV2UI_Controller = c_void_p
LV2UI_Handle = c_void_p

LV2UI_Write_Function = CFUNCTYPE(
    c_void_p,                
    LV2UI_Controller,  # First parameter: LV2UI_Controller (void*)
    c_uint32,          # Second parameter: uint32_t port_index
    c_uint32,          # Third parameter: uint32_t buffer_size
    c_uint32,          # Fourth parameter: uint32_t protocol
    POINTER(c_void_p)  # Fifth parameter: const void* buffer
)

# Define the structure according to the LV2 specification
class LV2UI_Descriptor(Structure):
    _fields_ = [
        ("uri", c_char_p),             # const char* uri
        ("instantiate", CFUNCTYPE(LV2UI_Handle,  # LV2UI_Handle (*instantiate)(
            c_void_p,                   #   const LV2UI_Descriptor*,
            c_char_p,                   #   const char* plugin_uri,
            c_char_p,                   #   const char* bundle_path,
            LV2UI_Write_Function,       #   LV2UI_Write_Function write_function,
            c_void_p,                   #   LV2UI_Controller controller,
            c_void_p,                   #   LV2UI_Widget* widget,
            POINTER(POINTER(LV2_Feature)) #   const LV2_Feature* const* features)
        )),
        ("cleanup", CFUNCTYPE(None, LV2UI_Handle)),  # void (*cleanup)(LV2UI_Handle)
        ("port_event", CFUNCTYPE(None,      # void (*port_event)(
            LV2UI_Handle,                   #   LV2UI_Handle,
            c_uint32,                       #   uint32_t port_index,
            c_uint32,                       #   uint32_t buffer_size,
            c_uint32,                       #   uint32_t format,
            POINTER(c_void_p)               #   const void* buffer)
        )),
        ("extension_data", CFUNCTYPE(c_void_p, c_char_p))  # const void* (*extension_data)(const char* uri)
    ]

class LV2_Atom(Structure):
    _fields_ = [
        ("size", c_uint32),  # Size of the atom body
        ("type", LV2_URID)   # Type of the atom
    ]

#class LV2_Atom_Event(Structure):
#    _fields_ = [
#        ("time", c_uint32),  # Time in frames or ticks
#        ("body", c_uint32)   # Offset to the atom body
#    ]

class LV2_Atom_Sequence_Body(Structure):
    _fields_ = [
        ("unit", c_uint32),  # Time unit (e.g., frames)
        ("pad",  c_uint32)    # Padding, must be 0
    ]

# Define LV2_Atom_Sequence structure
class LV2_Atom_Sequence(Structure):
    _fields_ = [
        ("atom", LV2_Atom),                  # Base atom header
        ("body", LV2_Atom_Sequence_Body)     # Sequence body
    ]
